  ...
  return (
    <>
      <div className="px-5 mt-7 mb-6">
        <h1 className="text-[32px] font-bold text-black">{t("measurementSection")}</h1>
      </div>

      <div className="flex flex-col gap-5 px-5">
        {!isArea && (
          <div className="flex flex-col gap-2">
            <label className="text-base font-semibold text-text-primary">{t("measurementTypeLabel")}</label>
            <div className="flex gap-2">
              {(["VOLUME", "MASS"] as const).map((type) => (
                ...
                >
                  {type === "VOLUME" ? t("volumeUnit") : t("weightUnit")}
                </button>
              ))}
            </div>
          </div>
        ...
        <div className="flex flex-col gap-2">
          <label className="text-base font-semibold text-text-primary">
            {t("amountLabel")} {!isArea && <span className="text-[#8f8f8c] font-normal">({unit})</span>}
          </label>
        ...
        {!isArea && (
          <div className="flex flex-col gap-2">
            <label className="text-base font-semibold text-text-primary">{t("descriptionLabel")}</label>
            <textarea
              value={form.impactDescription}
              onChange={(e) => update("impactDescription", e.target.value)}
              placeholder={t("measurementDescPlaceholder")}
              rows={4}
